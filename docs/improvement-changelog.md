...
| 15 - Static release gate | Added Checkstyle and SpotBugs to Maven `verify` and GitHub Actions so source-rule violations and bytecode defect patterns fail the same reproducible gate as tests and coverage. Suppressions are narrowly scoped and justified in their configuration files. | `clean verify` passes 437 tests (416 unit, 21 HTTP), with zero Checkstyle violations at the configured error threshold and zero non-excluded SpotBugs findings at the configured medium-or-higher threshold; CI preserves both reports as artifacts. | Kept. Static analysis belongs in the release gate rather than depending on an optional local inspection. |
